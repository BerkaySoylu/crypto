//Matris satýr, sütun iþlemleri kullanarak anahtarlý þifreleme örneði ve dosya iþlemleri. Þifrelenen metni girilen keye göre öteler.
#include <iostream>
#include <string>
#include <sstream>
#include <iterator>
...
void Crypto::encrypt(){
	...
	ofstream dosyaYaz("keyBasedEncrypted.txt");
  if ( dosyaYaz.is_open() ){
...
void Crypto::decrypt(){
	...
	cout<<"\nLutfen Keyi giriniz=";
	cin>>key;cin.ignore();
	ifstream dosyaOku("keyBasedEncrypted.txt");
	char encMetin;
	char enc[100];
	...
	for(int j=0;j<satirS-1;j++)
	{
		...
		{	
			if(enc[sayac]==' ' || enc[sayac]=='\n')
			cout<<enc[sayac];
			else if(enc[sayac]!=':')
			harf=enc[sayac];harf-=key;cout<<harf;	
			sayac+=satirS;
		}
		sayac=j+1;
		cout<<" ";
